...
На 1-м компьютере создадим текстовый файл \code{signed\_message.txt} c текстом \code{Hello from Anton Lamtev!} (листинг \ref{lst:signed-message.txt}). Затем выполним электронную подпись данного файла. Данный процесс представлен в листинге \ref{lst:sign-message.ub}.
